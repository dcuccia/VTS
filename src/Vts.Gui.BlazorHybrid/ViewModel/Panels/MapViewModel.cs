...
using Vts.Gui.BlazorHybrid.Helpers;
...
namespace Vts.Gui.BlazorHybrid.ViewModel
{
    public class MapViewModel : BindableObject
    {
        ...
        public byte[] ColorBar { get; private set; }
        ...
        public static MapViewModel Clone(MapViewModel mapToClone)
        {
            ...
            output.Bitmap = mapToClone.Bitmap != null ? mapToClone.Bitmap.ToArray() : null;
            output.ColorBar = mapToClone.ColorBar != null ? mapToClone.ColorBar.ToArray() : null;
        ...
        private void Map_DuplicateWindow_Executed(object sender, EventArgs e)
        {
            var vm = Clone();
            //MainWindow.Current.Main_DuplicateMapView_Executed(vm);
        }

        private void Maps_ExportDataToText_Executed(object sender, EventArgs e)
        {
        ...
        public void UpdateImages()
        {
            ...
            //}
            var colorMapData =
                (from i in Enumerable.Range(0, ColorBar.Length)
                 let pixel = GetGrayscaleColor(i, 0, ColorBar.Length - 1)
                 let buffer = BitConverter.GetBytes(pixel)
                 from j in buffer
                 select j
                ).ToArray();
            //source = BitmapSource.Create(256, 1, 96, 96, PixelFormats.Bgra32, null, colorMapData, stride);
            ColorBar = colorMapData; //new WriteableBitmap(source);
