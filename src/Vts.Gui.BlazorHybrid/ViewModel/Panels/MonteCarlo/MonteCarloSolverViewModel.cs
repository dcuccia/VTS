...
using Vts.Gui.BlazorHybrid.Model;
using Vts.IO;
using Vts.MonteCarlo;
using Vts.MonteCarlo.Detectors;
using Vts.MonteCarlo.IO;
using Vts.Gui.BlazorHybrid.Extensions;
using Vts.MonteCarlo.Tissues;

namespace Vts.Gui.BlazorHybrid.ViewModel
{
    /// <summary>
    ...
    public class MonteCarloSolverViewModel : BindableObject
    {
        ...
        private SimulationInput _inputCache;
        private SimulationOutput _outputCache;
        ...
        private async void MC_ExecuteMonteCarloSolver_Executed(object sender, EventArgs e)
        {
        ...
        private bool MC_InfileIsValidForGUI(SimulationInput input)
        {
            ...
            if ((input.DetectorInputs.All(d => d.TallyType != "ROfRho")) &&
                (input.DetectorInputs.All(d => d.TallyType != "FluenceOfRhoAndZ")))  
            {
        ...
        private void MC_CacheSimulationResults(SimulationInput input)
        {
            ...
            _inputCache = input;
            _outputCache = _output;
            logger.Info(() => StringLookup.GetLocalizedString("Message_Done") + ".\r");
        }

        private async Task<bool> MC_SaveSimulationResultsFromCache()
        {
            var input = _inputCache;
            var output = _outputCache;
        ...
        private async void MC_CancelMonteCarloSolver_Executed(object sender, EventArgs e)
        {
            CanCancelSimulation = false;
            CanRunSimulation = true;
            CanLoadInputFile = true;
        ...
        private async void MC_LoadSimulationInput_Executed(object sender, EventArgs e)
        {
            CanRunSimulation = false;
            CanLoadInputFile = false;
        ...
        private async void MC_DownloadDefaultSimulationInput_Executed(object sender, EventArgs e)
        {
            //using (var dialog = new FolderBrowserDialog())
            //{
            //    var dialogResult = dialog.ShowDialog();
        ...
        private async void MC_SaveSimulationResults_Executed(object sender, EventArgs e)
        {
